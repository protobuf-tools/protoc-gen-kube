...
func gen() error {
	...
	gctxt, err := generator.NewContext(builder, kubetype.NameSystems("" /* generatedPackage */, nil), kubetype.DefaultNameSystem())
	if err != nil {
	...
	// scan packages
	packages := scanner.New(ctx, gctxt, arg).Packages()

	// execute packages
	if err := gctxt.ExecutePackages(arg.OutputBase, packages); err != nil {
		logf.Error(err, "executing generator")
